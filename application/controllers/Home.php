...
{
  $data["page"]  = "Test" ;
  $data["quiz"] = $this->db->query("select * from quiz order by id desc")->row();
  $data["questions"] = $this->db->query("select * from quiz_questions where quiz_id = ".$data["quiz"]->id." order by id asc")->result() ;
...
public function privacy_policy()
{
   ...
   $data["detail"] = '';//$this->Home_model->detail("privacy",1);
...
public function termsandcondition()
{
   ...
   $data["detail"] = '';//$this->Home_model->detail("terms",1);
...
public function selectplan($id = '')
{
  ...
  if($id)
  {
    $data["page"] = "Review Plan";
    $data["result_id"] = $id;
    $data["plans"] = $this->Home_model->selectdata("plans");
...
public function submitquestion()
{
    ...
    {
        $qq = $this->Home_model->detail("quiz_questions",$val);
        $correct = $qq->correct_answer ;
        
        if($_POST["category"][$key] == "abstract_resoning"){ $abstract_resoning++ ; }
        if($_POST["category"][$key] == "pattern_recognition"){ $pattern_recognition++ ; }
        if($_POST["category"][$key] == "deductive_reasoning"){ $deductive_reasoning++ ; }
        if($_POST["category"][$key] == "numerical_reasoning"){ $numerical_reasoning++ ; }
        if($_POST["category"][$key] == "cognitive_ability"){ $cognitive_ability++ ; }
        
        
        
        $u_ans = $this->input->post("question".$val)?:'';
        if($u_ans == $correct)
        {
          $correct_answer++ ;
        }
    ...
    $score1 = (150) - ($result->wrong_answer * 7.5) ;
    $score=round($score1);


    $id =  $this->Home_model->insert("user_result",array("quiz_id"=>$quiz_id,
                                                  "total_question"=>$total_question,
                                                  "correct_answer"=>$correct_answer,
                                                  "wrong_answer"=>$wrong_answer,
                                                  "score"=>$score,
                                                  "abstract_resoning"=> $abstract_resoning,
    ...
    echo base_url('selectplan/'.$id) ;
}


public function payment($planid='',$resultid ='')
{
  $store = $this->Home_model->detail("admin_user",1); 
  
   if($this->input->post('bookingSubmit'))
  {
      if(isset($_GET['role']) && $_GET['role'] =="other"){
            $chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ";
            $coupons = "";
            $plan = $this->Home_model->detail("plans",$planid);
            for ($i = 0; $i < 10; $i++) {
                $coupons .= $chars[mt_rand(0, strlen($chars)-1)];
            }
            $user_result = $this->Home_model->detail("user_result",$resultid);
            //print_r($user_result->name.$user_result->email);exit;
     ...
     try 
      {
        Stripe::setApiKey($store->stripe_sk);
        //Stripe::setApiKey("sk_test_51LoeCLSAQZSYt1m5SbnoOr8XeRU2tGOg4BmmIt9klUf38EHkuhvojT0Ady3THHOU3AuTHft4Kc34PopUHIZXLBwZ00di9iFR77");
        //Stripe::setApiKey('sk_live_u3aPXmrlxLdhO7x8Qucm5MDj');
        $charge = Stripe_Charge::create(array(
              "amount" => intval($plan->price)*100 ,
              "currency" => 'usd',
              "card" => $this->input->post('stripeToken'),
              "description" => "For Buy ".$plan->name
        ));
        ...
        if($status == "true")
        {
          ...
          $tran = array("result_id"=>$resultid,
                        "user_id"=>$buyer["name"],
                        "amount"=>$plan->price,
                        "plan_id"=>$planid,
                        "transaction_id"=>$this->input->post('stripeToken'),
                        "payment_method" => "stripe",
                        "purchase_coupons" => "1",
                        "payment_status"=>1,
                        "created_on"=>date("Y-m-d H:i:s")); 
           $this->Home_model->insert("transaction",$tran) ; 
            $id= $this->Home_model->insert("coupons",array(
                                                  "name"=>$this->input->post('name'),
                                                  "email"=>$this->input->post('email'),
                                                  "coupon_data"=>$coupons,
                                                ));
           $trans = array("payment_status"=>1,"status"=>1); 
        ...
        $pdf_name="coupons-from".$this->input->post('name');
       //echo"<pre>";print_r($data['payment']);die;
        $html = $this->load->view('genrate_coupon',$data,true);
        $mpdf->WriteHTML($html);
        $mpdf->Output($pdf_name.'.pdf','D'); // opens in browser
           */
           $filename = time()."_coupon.pdf";
...
$this->m_pdf->pdf->Output($filename, "I");
           $this->session->set_flashdata("success","Coupons created successfully ..!") ;
           redirect(base_url()) ;
          exit;
        } 
        else
        {
          $this->session->set_flashdata("message","Payment Canceled By Your Bank..!") ;
          ?> 
            <script>location.assign("<?php echo base_url('Home/payment/'.$planid.'/'.$resultid) ; ?>") ; </script> <?php   
        }
      } catch (Stripe_CardError $e) {
        $this->session->set_flashdata("message","Payment Failed Due to card Reason ..!") ;
        ?> <script>  location.assign("<?php echo base_url('Home/payment/'.$planid.'/'.$resultid.'/?msg=Payment Failed Due to card Reason ..!') ; ?>") ; </script> <?php   
        exit();
      } catch (Stripe_InvalidRequestError $e) {
       //echo "<pre>"; print_r($e); exit;
        // Invalid parameters were supplied to Stripe's API
        $this->session->set_flashdata("message","Payment Failed Due to Internalreason..!") ;
        ?> <script> location.assign("<?php echo base_url('Home/payment/'.$planid.'/'.$resultid.'/?msg=Payment Failed Due to Internal Reason..!') ; ?>") ; </script> <?php   
        exit();
      } catch (Stripe_AuthenticationError $e) {
        // Authentication with Stripe's API failed
        $this->session->set_flashdata("message","Payment Failed Due to Invalid autentication ..!") ;
        ?> <script> location.assign("<?php echo base_url('Home/payment/'.$planid.'/'.$resultid.'/?msg=Payment Failed Due to Invalid autentication ..!') ; ?>") ; </script> <?php   
        exit();
      } catch (Stripe_ApiConnectionError $e) {
        // Network communication with Stripe failed
        $this->session->set_flashdata("message","Payment Failed Due to api Reason ..!") ;
        ?> <script> location.assign("<?php echo base_url('Home/payment/'.$planid.'/'.$resultid.'/?msg=Payment Failed Due to api Reason ..!') ; ?>") ; </script> <?php   
        exit();
       } catch (Stripe_Error $e) {
        // Display a very generic error to the user, and maybe send
        $this->session->set_flashdata("message","Payment Failed Due to server problem ..!") ;
        ?> <script> location.assign("<?php echo base_url('Home/payment/'.$planid.'/'.$resultid.'/?msg=Payment Failed Due to server problem ..!') ; ?>") ; </script> <?php   
        exit();
      } catch (Exception $e) {
    ...
    if($planid && $resultid)
    {
      $result = $this->Home_model->detail("user_result",$resultid);
      $plan = $this->Home_model->detail("plans",$planid);

      $paypal_pay = $this->input->post('totalamount') ;
      ...
      $this->Home_model->update("user_result",$resultid,$buyer);
      ...
      $carddetail =  array(
        ...
        if($this->input->post('coupons') != ""){
           $cp= $this->input->post('coupons');
           $coupon = $this->db->query("select * from coupons where coupon_data='$cp'")->row();
           //print_r($coupon);exit;
               if($coupon !="" && $coupon->coupon_data == $cp && $coupon->email != $this->input->post('email')){
                    json_encode(array('status'=>200,'success'=>'Payment successfully completed.'));
                     // echo "Payment Successfully Completed..!";
                      ...
                      $tran = array("result_id"=>$resultid,
                       ...
                       $trans = array("payment_status"=>1,"status"=>1); 
                       $this->Home_model->update("user_result",$resultid,$trans) ; 
                       $this->db->query("DELETE FROM coupons WHERE coupon_data='$cp'");
                       
                       $this->successmail($resultid);
                       $this->session->set_flashdata("success","Payment Successfully completed ..!") ;
                       redirect(base_url('Home/afterpayment/'.$planid.'/'.$resultid)) ;
               }else{
                 $this->session->set_flashdata("message","Coupons not vailid or email already exist ..!") ;
                 ?> <script> location.assign("<?php echo base_url('Home/payment/'.$planid.'/'.$resultid.'/?msg=Coupons not vailid or email already exist ..!') ; ?>") ; </script> <?php   
                 exit();
               }
        }else{
      try 
  ...
  $data["result"] = $this->Home_model->detail("user_result",$resultid);
   if(isset($_GET['role']) && $_GET['role'] =="other"){
       $data["plan"] = $this->Home_model->detail("plans2",'1');
    }else{
  $data["plan"] = $this->Home_model->detail("plans",$planid);
    }
  $data["store"] = $store ;
...
{
  $data["page"] = "After Payment";
  $this->load->view("afterpayment",$data);
}

public function certificate($rid = ''){
    $filename = time()."_certificate.pdf";

// unpaid_voucher is unpaid_voucher.php file in view directory and $data variable has infor mation that you want to render on view.
//$data['coupon'] = $coupons;
$admin = $this->Home_model->detail("admin_user",1);
  $other_plan = $this->Home_model->detail("plans2",1);
  $data['result'] = $this->Home_model->detail("user_result",$rid);
  $total='150';
$score1 = ($total) - ($data['result']->wrong_answer * 7.5);
//print_r($score1);die;
$data['score']=round($score1);
$this->load->library('M_pdf');
...
public function successmail($rid = '')
{
  $admin = $this->Home_model->detail("admin_user",1);
  $other_plan = $this->Home_model->detail("plans2",1);
...
$score=round($score1);
$cdate = date("m/d/Y"); 
$other_plans = $this->Home_model->wheredetail("transaction","result_id",$result->id);
$plan = $this->Home_model->detail("plans",$other_plans->plan_id);
$price = $plan->price ;
 ?>
<!DOCTYPE html>
<html lang="en">
...
<div class="users-row">
  ...
  <tr>
    ...
    <?php  if($other_plans->plan_id == 2 || $other_plans->plan_id == 3)
{ ?>
    <tr>
     <td style="padding:10px;">
     <div style="background:#fff; border-radius:10px; padding:15px;">
       <table width="100%" border="0" cellspacing="0" cellpadding="0">
         <tr>
           ...
           <p><strong>$<?php echo $other_plan->price; ?> only</strong></p>
            ...
            <td align="left" valign="top" style="padding:10px; width:50%;">
                <div style="background:#fff; border-radius:10px; padding:15px;">
                    ...
                    <script>
                        ...
                        var barColorsgc = ["#9164fb","#9164fb","#9164fb","#9164fb","#9164fb","#9164fb","#9164fb","#9164fb","#9164fb","#9164fb"];
...
// $this->m_pdf->pdf->WriteHTML($msg);

// //download it D save F.

// $this->m_pdf->pdf->Output("upload/result/".$filename, "F");
...
<title>Report</title>
<!-- Favicon -->
<link rel="icon" href="'.base_url("result/").'images/favicon.png">
<style type="text/css">
body,td,th {
	font-size: 16px;
	line-height:22px;
...
body {
	background-color: #dee7ea; padding:80px;
}
canvas{width: 100% !important;}
img { display:block; }
</style>
...
<div>
  ...
  <tr>
      <td style="padding:10px;">
      <h3 style="margin:0px 0px 6px 0px; font-size:13px; text-transform:uppercase; color:#627182;">Results</h3>
      <h2 style="margin:0px; font-size:26px;">'.$result->name.'</h2>
      </td>
    </tr>
    <tr>
      <td style="padding:10px;">&nbsp;</td>
        ...
        <tr>
          <td style="padding:10px; width:25%;">
          	<div style="background:#fff; border-radius:10px; padding:15px;">
          	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
          	    <tr>
          	      <td width="63" align="left" valign="top"><img src="'.base_url("result/").'images/icon1.jpg"></td>
          	      <td width="190" align="left" valign="top">
                  <h3 style="margin:0px 0px 6px 0px; font-size:13px; text-transform:uppercase; color:#627182;">IQ Score</h3>
                  <h2 style="margin:0px; font-size:26px;">'.$score.'</h2>
                  </td>
        	      </tr>
        	    </table>
          	</div>
          ...
          <td style="padding:10px; width:25%;">
          	<div style="background:#fff; border-radius:10px; padding:15px;">
          	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
          	    <tr>
          	      <td width="63" align="left" valign="top"><img src="'.base_url("result/").'images/icon2.jpg"></td>
          	      <td width="190" align="left" valign="top">
                  <h3 style="margin:0px 0px 6px 0px; font-size:13px; text-transform:uppercase; color:#627182;">Date</h3>
                  <h2 style="margin:0px; font-size:26px;">'.$cdate.'</h2>
                  </td>
        	      </tr>
        	    </table>
          	</div>
          ...
          <td style="padding:10px; width:25%;">
          	<div style="background:#fff; border-radius:10px; padding:15px;">
          	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
          	    <tr>
          	      <td width="63" align="left" valign="top"><img src="'.base_url("result/").'images/icon3.jpg"></td>
          	      <td width="190" align="left" valign="top">
                  <h3 style="margin:0px 0px 6px 0px; font-size:13px; text-transform:uppercase; color:#627182;">Categories</h3>
                  <h2 style="margin:0px; font-size:26px;">'.$result->quiz_id.'</h2>
                  </td>
        	      </tr>
        	    </table>
          	</div>
...
<p>This IQ exam evaluates, among other aspects, logical logic, math skills, spatial communication skills, knowledge recall, and the ability to solve novel problems. It does not take social or emotional considerations into consideration. </p>
    ...
    <tr>
      ...
      <h3 style="font-size:18px; border-bottom:1px solid #dee7ea; padding-bottom:15px;">What does it measure?</h3>
      <div style="min-height:377px;">
      <p>If you have recently completed or partially completed our IQ Test, you are probably curious about the results and want to hear more about what they say. After all, the results of an IQ test pose a slew of concerns, such as what is the average IQ? Is my performance indicative of high intelligence, low intelligence, or a combination of the two? How are the results of IQ experiments determined? Both of these questions, as well as a number of others, can be addressed by understanding how IQ is measured and what the mean average score is for each category.
</p> 
<p> It is also worth noting that IQ test results are graded, and that the performance would be somewhere between "cognitively impaired" at the bottom and "highly talented" at the top of the scale. </p>
</div>
...
The average score at the IQ Test Academy is 100 points, with a standard deviation of 15 points above and below this baseline - 115 and 85 points, respectively. This is the group that 68 percent of people would fall under. We have a variety of distinct categories of intelligence on each side of this category, with a declining proportion of the population the more you go to either end of the scale. Indeed, 2.5 percent of the population has an IQ below 70, and 2.5 percent would reach a score over 130, with just 0.5 percent ranking over 140. </p>
<p>
It is interesting to note that, actually, such low and high scores are quite difficult to measure using standard intelligence tests, and very high IQ scores are especially hard to determine in an accurate fashion. This is due to the fact that a lot of reference measurements are required in order to determine specific scores reliably, and as very high and very low IQ scores are uncommon and unusual - in the sense that relatively very few people achieve them - reliable reference groups to make use of are difficult to formulate. </p>
...
The horizontal axis of the Bell Curve depicts all of the possible scores a test taker might get, from the lowest to the best. The number of people who obtain a given score is represented by the vertical axis. The distinctive "bell curve" pattern that occurs is due to the fact that the maximum number of individual scores will always be at the mean average of 100, with the standard deviation of 15 points marking the start of the downward curve on each side of the top. The curve steepens as the scores rise and fall, until only a small percentage of people are represented at either end. 
    ...
    <tr>
      <td ><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td  valign="top" style="padding:10px; width:40%;"><div style="background:#fff; border-radius:10px; padding:15px;">
            <h3 style="font-size:18px; border-bottom:1px solid #dee7ea; padding-bottom:15px;">Overall Score</h3>
           <div style="min-height:377px;">  
           <p>• 141 or more: Very gifted or highly advanced <br>
           • 130-140: Gifted or very advanced  <br>
    ...
    <tr>
      <td style="padding:10px;">
      <div style="background:#fff; border-radius:10px; padding:15px;">
      <h3 style="color:#9066fb; font-size:16px; border-bottom:1px solid #dee7ea; padding-bottom:15px;">IQ Test Results 131 - 140</h3>
      
<p>If your IQ score is between 131 and 140, you are one of the very few who can claim to be "gifted." This suggests you should be proud of yourself because, in contrast to your peers, you have a high intelligence quotient, having performed exceedingly well on the IQ Test. Why did you do so well on this IQ test? Okay, there may be a number of reasons for this. You may have worked hard to pass the major puzzles on the IQ exam, which are spatial and pattern-based. These forms of logic puzzles can be a natural ability - or "gift," as the groups name implies - for you. Many psychologists agree that evolutionary conditions contribute to a large portion of talented IQs, so you might have inherited any of the inherent ability to bend the mind in the ways that the test demands. Whatever the reason for your certified IQ test score of 131 to 140, it is reasonable to believe that such a score suggests a considerable potential for academic and job accomplishment, and that your giftedness would assist you in achieving a bright and fruitful future! </p>
<p><strong>What does it mean to be gifted?</strong></p> 
<p>The "gifted" grade, which normally falls between 131 and 140 points on the IQ scale, is probably the most intriguing and deserving of further investigation. It is crucial to note that an IQ test score of 131 to 140 places you easily above the average level of intellect, with a mean average score of 100 points and a standard deviation of plus or minus fifteen points. In recent years, among IQ testers, the word "gifted" has supplanted other terms such as "intelligent" (which was considered misleading) and "low-level genius" (which, again, could be confusing). One might contend that the word "gifted" also does not exactly suit the bill: it implies a natural ability, a "gift" that one is either born with or without... Many scientists believe that this is deceptive in and of itself.</p>

<p>Although there is proof that hereditary characteristics contribute to higher IQ levels, this is only realized by the implementation of diligent work, concentration, initiative, and applied intellect. That is to suggest, anyone might be born with the potential to achieve a high IQ test score, just like they may be born with the potential to become a champion pole-vaulter or an oil painter, but without the necessary preparation and effort to realize that potential, nothing especially spectacular or exceptional may emerge.</p> 
<p>What Percentage of the Population Has a Gifted IQ Score? It should come as no surprise that when we step closer to either end of the IQ test bell curve, less and fewer participants, and a smaller and smaller portion of the population, appear in the higher and lower score groups. The talented group, identified as those who score between 131 and 140 on an IQ exam, accounts for just 2% of the population, placing them in the minority.</p> 
    ...
    <tr>
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td  valign="top" style="padding:10px; width:60%;"><div style="background:#fff; border-radius:10px; padding:15px;">
            <h3 style="font-size:18px; border-bottom:1px solid #dee7ea; padding-bottom:15px;">Pattern recognition</h3>
            <div style="min-height:377px;">
            <p> The method of detecting, distinguishing, and categorizing complicated arrangements of sensory objects into organized schemes in a way that enables memory encoding and retrieval is known as pattern recognition. It is a natural and unconscious phenomenon that determines the ability to perceive order in unpredictable situations. Since it determines your capacity to think objectively and your potential to understand and make use of logical sequences, pattern recognition is thought to be particularly related to your General Intelligence standard. </p>
<p>The "Pattern Recognition" collection of questions is intended to assess how readily and reliably you can understand the underlying mechanics of various scenarios and make correlations between them. </p>
...
<p>Cognitive ability is an essential feature of human intellect and a crucial ability for learning. People who have a well-developed visual vision are thought to be stronger and quicker learners. </p>
    ...
    <tr>
      <td style="padding:10px;"><div style="background:#fff; border-radius:10px; padding:15px;">
        <h3 style="color:#9066fb; font-size:16px; border-bottom:1px solid #dee7ea; padding-bottom:15px;">Conclusion</h3>
        <p>We hope that now that you have completed the IQ test and have a better understanding of where you fall on the hierarchy of test outcome groups, you haveve developed a better understanding of your own intelligence quotient and how it relates to those around the world. </p>
        
        <p>Knowing and recognizing your IQ has many advantages, whether you are on the bottom end of the spectrum, in the centre with an average score, or have discovered proof that you are mentally gifted or on the verge of becoming a genius. It will assist you in determining which career options or college programs are right for you. It will help you figure out where you shine and where you need to develop. It will also assist you in determining the next move in life and determining your real intellectual capacity. Any online test remains for informative purposes and should not be relied upon or used as the sole basis for making decisions without reviewing initial, more accurate, more detailed, or more timely sources of knowledge in real life. </p>
        
<p><strong>Will My IQ Test Result Change?</strong></p>

<p>Your IQ test score, like anything else in existence, is not set in stone. While most peoples IQ test scores plateau about the age of 18, and then tend to decline slightly after the age of 65, there is plenty of data that suggests IQ test scores can be improved. If you believe you should have performed well on your IQ test and wish to be placed in a higher level of test scores, you will be relieved to learn that there are many options available to you.</p>
...
<p>If you took the IQ Test out of curiosity, for science testing, to answer questions about your academic or career future, or for pure amusement, we hope it gave you the answers you were looking for. </p>
    ...
    <tr>
      <td style="padding:10px;">&nbsp;</td>
		  ...
		  $web_email = $this->db->get('admin_user')->row()->web_email ;
		  $this->load->library('email',$config);
		  $this->email->from($web_email, $admin->username);
		  $this->email->to($result->email); 
		  $this->email->subject('Smart IQ Academy');   
		  $this->email->message($msg2);  
		  //$this->email->attach(base_url()."upload/result/".$filename);
	      $this->email->send();
     ...
     /* $to = $result->email ;
      ...
      $headers  = "From: ".$admin->username." <" . strip_tags($admin->web_email) . ">\r\n";
